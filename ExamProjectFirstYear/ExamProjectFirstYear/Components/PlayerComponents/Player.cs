...
namespace ExamProjectFirstYear
{
    ...
    public class Player : Component, IGameListener
    {
        ...
        public void Notify(GameEvent gameEvent, Component component)
        {
            ...
            if (gameEvent.Title == "Colliding" && component.GameObject.Tag == Tag.MATERIAL)
            {
                ...
                GameWorld.Instance.sQLiteHandler.IncreaseAmountStoredMaterial(componentMaterial.MaterialID, 
                                                                              GameWorld.Instance.inventory.InventoryID);
            }
        ...
        private void LoadSave()
        {
            if (saveLoaded == false)
            {
                InventoryID = GameWorld.Instance.sQLiteHandler.SelectIntValuesWhere("InventoryID", "Journal", 
                            $"ID={GameWorld.Instance.journal.JournalID}", 
                              new SQLiteConnection(GameWorld.Instance.sQLiteHandler.LoadSQLiteConnectionString()));
                Health      = GameWorld.Instance.sQLiteHandler.SelectIntValuesWhere("Health", "Journal",
                            $"ID={GameWorld.Instance.journal.JournalID}",
                              new SQLiteConnection(GameWorld.Instance.sQLiteHandler.LoadSQLiteConnectionString()));
                OpenDoor     = GameWorld.Instance.sQLiteHandler.SelectIntValuesWhere("OpenDoor", "Journal",
                            $"ID={GameWorld.Instance.journal.JournalID}",
                              new SQLiteConnection(GameWorld.Instance.sQLiteHandler.LoadSQLiteConnectionString()));
                Mana         = GameWorld.Instance.sQLiteHandler.SelectIntValuesWhere("Mana", "Journal",
                            $"ID={GameWorld.Instance.journal.JournalID}",
                              new SQLiteConnection(GameWorld.Instance.sQLiteHandler.LoadSQLiteConnectionString()));
        ...
        public void TestMethod()
        {
            ...
            {
                //GameWorld.Instance.sQLiteHandler.UpdateTable("Journal", "OpenDoor=1", $"ID={PlayerID}", new SQLiteConnection(GameWorld.Instance.sQLiteHandler.LoadSQLiteConnectionString()));
            }
