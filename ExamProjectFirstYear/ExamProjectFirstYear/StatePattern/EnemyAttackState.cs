...
namespace ExamProjectFirstYear.StatePattern
{
    class EnemyAttackState : IState
    {
        ...
        {
            //NodeManager.Instance.DebugPath.RemoveRange(0, NodeManager.Instance.debugcount);

            //if ((enemy as FlyingEnemy).FlyingPath != null)
            //{
            //    (enemy as FlyingEnemy).FlyingPath.Clear();
            //}


            //if ((new Vector2((int)(enemy.Target.Transform.Position.X / NodeManager.Instance.CellSize), (int)(enemy.Target.Transform.Position.Y / NodeManager.Instance.CellSize)) != new Vector2(0, 6)))
            //{
            ...
            //if ((enemy as FlyingEnemy).PrevTargetNode != new Vector2((int)(enemy.Target.Transform.Position.X / NodeManager.Instance.CellSize), (int)(enemy.Target.Transform.Position.Y / NodeManager.Instance.CellSize)))
            //{
            //    (enemy as FlyingEnemy).FlyingPath = (enemy as FlyingEnemy).EnemyPathFinder.FindPath
            //     (enemy.GameObject.Transform.Position,
            //     enemy.Target.Transform.Position);
            ...
            //(enemy as FlyingEnemy).PrevNode = new Vector2((int)enemy.Target.Transform.Position.X / NodeManager.Instance.CellSize, (int)enemy.Target.Transform.Position.Y / NodeManager.Instance.CellSize);
            ...
            //NodeManager.Instance.debugcount = ((enemy as FlyingEnemy).FlyingPath.Count);




            /*
        for (int i = 0; i <= (enemy as FlyingEnemy).FlyingPath.Count; i++)
        {
            NodeManager.Instance.DebugPath.RemoveAt(i);
